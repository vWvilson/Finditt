...
var _default = {
  search: function search(searchTerm, searchLimit, sortBy) {
    return fetch("http://www.reddit.com/search.json?q=".concat(searchTerm, "\n        &sort=").concat(sortBy, "&limit=").concat(searchLimit)) // first .then gives us the response, then we tell it we want the data in json
    .then(function (res) {
      return res.json();
...
searchForm.addEventListener('submit', function (e) {
  ...
  var searchLimit = document.getElementById('limit').value; // check for input
...
var showMessage = function showMessage(message, className) {
  // create div
  var div = document.createElement('div'); // create bootsrap class for alert

  div.className = "alert ".concat(className); // add alert text

  div.appendChild(document.createTextNode(message)); // get the parent container to be able to put it above it

  var searchContainer = document.getElementById('search-container'); // insert message

  searchContainer.insertBefore(div, _redditapi.default); // time out the alert div
...
//# sourceMappingURL=/findditt.e31bb0bc.js.map